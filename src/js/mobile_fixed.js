  ...
  try {
    ...
    if (faces.size() > 0) {
      ...
      if (lastFaceRect) {
        ...
        bestFace.x = Math.round(alpha * lastFaceRect.x + (1 - alpha) * bestFace.x);
        bestFace.y = Math.round(alpha * lastFaceRect.y + (1 - alpha) * bestFace.y);
        bestFace.width = Math.round(alpha * lastFaceRect.width + (1 - alpha) * bestFace.width);
        bestFace.height = Math.round(alpha * lastFaceRect.height + (1 - alpha) * bestFace.height);
      }
...
function extrapolateCardRectangle(faceRect) {
  ...
  // Calculer les dimensions de la carte en fonction de la taille du visage
  const cardWidth = faceRect.width * 2.5;
  const cardHeight = cardWidth / CARD_RATIO;

  // Positionner la carte sous le visage
  const cardX = faceRect.x + faceRect.width / 2 - cardWidth / 2;
  const cardY = faceRect.y + faceRect.height * 1.3;
...
async function analyzeImageQuality(gray) {
  try {
    ...
    const mean = new openCv.Scalar();
    const stddev = new openCv.Scalar();
    openCv.meanStdDev(gray, mean, stddev);
    const brightness = Math.round(mean.v0);

    // Calculer la variance (mesure de netteté)
    const variance = Math.round(stddev.v0 * stddev.v0);
