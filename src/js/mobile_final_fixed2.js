...
async function detectEyes(src, faceRect) {
  ...
  try {
    ...
    faceROI = src.roi(eyeRegionRect);
    grayROI = new openCv.Mat();
    openCv.cvtColor(faceROI, grayROI, openCv.COLOR_RGBA2GRAY);

    // Détecter les yeux
    eyes = new openCv.RectVector();
...
function capturePhoto(mode = "auto") {
  ...
  try {
    ...
    if (video.readyState === 4) {
      ...
      // Capturer exactement le contenu du rectangle bleu (carte)
      photoCtx.drawImage(
        video,
        highResCardRect.x,
        highResCardRect.y,
...
function retakePhoto() {
  ...
  // Relancer le traitement vidéo s'il s'était arrêté
  requestAnimationFrame(processFrame);
}
...
function confirmPhoto() {
  ...
  // Vous pourriez ajouter ici le code pour envoyer la photo à un serveur
  // Exemple:
  // fetch('/upload', {
  //   method: 'POST',
...
async function processFrame() {
  try {
    if (!openCv || !video.readyState || video.readyState < 2 || photoTaken) {
      if (!photoTaken) {
        requestAnimationFrame(processFrame);
      }
      return;
